...
private:
	...
	float lastTime = 0.0f;
public:
	void create(const engine::event_object& event) override
	{
	...
	UINT update(const engine::event_object& event, const SIZE& size) override
	{
...
bool default_handler(const engine::event_object& event)
{
	switch (event.type())
	{
		case WM_CREATE:
		{
			DragAcceptFiles(event.window().hwnd(), true);
			event.window().text(L"Simple");
			event.window().show(SW_SHOW);
			return true;
		}
...
bool engine::main_handler(const engine::event_object& event)
{
	bool result = false;

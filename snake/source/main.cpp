...
{
public:
	timer_class() noexcept : _current(engine::clock()), _delta(0), _elapsed(0)
	{
	}
public:
...
private:
	double _current, _delta, _elapsed;
};
...
private:
	...
	{
		const auto new_rect = D2D1::RectF(rect.left, rect.top, rect.left + rect.right, rect.top + rect.bottom);
		this->_render_target->DrawTextW(string.data(), string.size(), format.Get(), new_rect, brush.Get());
	}
	...
	bool handler(HWND window, UINT message, WPARAM wparam, LPARAM lparam) override
	{
		switch (message)
		{
			case WM_CREATE:
			{
				engine::failed_code(D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, this->_factory.GetAddressOf()));
				engine::failed_code(DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, __uuidof(IDWriteFactory), reinterpret_cast<IUnknown**>(this->_write_factory.GetAddressOf())));
				engine::failed_code(this->_factory->CreateHwndRenderTarget(D2D1::RenderTargetProperties(), D2D1::HwndRenderTargetProperties(window), this->_render_target.GetAddressOf()));

				engine::failed_code(this->_render_target->CreateSolidColorBrush(D2D1::ColorF(0x003000), this->_text_brush.GetAddressOf()));
				engine::failed_code(this->_write_factory->CreateTextFormat(L"Arial", nullptr, DWRITE_FONT_WEIGHT_BOLD, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, 24.0f, L"", this->_text_format.GetAddressOf()));
				engine::failed_code(this->_text_format->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_CENTER));
				engine::failed_code(this->_text_format->SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT_CENTER));
				return true;
			}

			case WM_SIZE:
			{
				this->_render_target->Resize(D2D1::SizeU(GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam)));
				return true;
			}
			...
			case WM_PAINT:
			{
				...
				const auto window_size = this->_render_target->GetSize();
				const auto game_area = D2D1::RectF((window_size.width - GAME_AREA_SIZE_X) / 2, (window_size.height - GAME_AREA_SIZE_Y) / 2, GAME_AREA_SIZE_X, GAME_AREA_SIZE_Y);
				...
				engine::failed_code(this->_render_target->EndDraw());
				return true;
			}
...
int engine::main()
{
	const int width = main_window::GAME_AREA_SIZE_X, height = main_window::GAME_AREA_SIZE_Y;
	const int left = (GetSystemMetrics(SM_CXFULLSCREEN) - width) / 2, top = (GetSystemMetrics(SM_CYFULLSCREEN) - height) / 2;

	return main_window().run(L"Snake", WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX, left, top, width, height);
}

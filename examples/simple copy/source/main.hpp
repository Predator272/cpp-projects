...
namespace engine
{
	...
	public:
		virtual void create(const event_object& event)
		{
		}

		virtual UINT update(const event_object& event, const SIZE& size)
		{
			return 0;
		}

		bool handler(const event_object& event)
		{
			switch (event.type())
			{
				case WM_CREATE:
...
#define D3D11_CREATE_DEVICE_FLAGS 0
					...
					this->_create_swap_chain(event.window().hwnd(), event.window().size(), 1);
					this->create(event);
					return true;
				}
				...
				case WM_SIZE:
				{
					...
					{
						this->_create_swap_chain(event.window().hwnd(), size, 0);
					}
					return true;
				}
				...
				case WM_PAINT:
				{
					if (this->_swap_chain->Present(0, DXGI_PRESENT_TEST) == S_OK)
					{
						engine::throw_code(this->_swap_chain->Present(this->update(event, event.window().size()), 0));
						return true;
					}
					return false;
				}
